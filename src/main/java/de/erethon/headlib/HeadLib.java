...
public enum HeadLib {
    ...
    static HeadLib.InternalsProvider internals;

    private HeadLib(String uuid, String textureValue) {
        this.uuid = uuid;// 357
        this.textureValue = textureValue;// 358
    }// 359

    private Object getSkullOwner() {
        if (this.skullOwner == null) {// 362
            this.skullOwner = internals.createOwnerCompound(this.uuid, this.textureValue);// 363
        }

        return this.skullOwner;// 365
    }

    public void give(Player player) {
        player.getInventory().addItem(new ItemStack[]{this.toItemStack()});// 374
    }// 375

    public void give(Player player, int amount) {
        player.getInventory().addItem(new ItemStack[]{this.toItemStack(amount)});// 384
    }// 385

    public void give(Player player, String displayName, String... loreLines) {
        player.getInventory().addItem(new ItemStack[]{this.toItemStack(displayName, loreLines)});// 395
    }// 396

    public void give(Player player, int amount, String displayName, String... loreLines) {
        player.getInventory().addItem(new ItemStack[]{this.toItemStack(amount, displayName, loreLines)});// 407
    }// 408

    public ItemStack toItemStack() {
        return this.toItemStack(1);// 416
    }

    public ItemStack toItemStack(int amount) {
        return this.toItemStack(amount, (String)null);// 426
    }

    public ItemStack toItemStack(String displayName, String... loreLines) {
        return this.toItemStack(1, displayName, loreLines);// 437
    }

    public ItemStack toItemStack(int amount, String displayName, String... loreLines) {
        ItemStack item = internals.newPlayerHead(amount);// 449
        if (displayName != null) {// 451
            ItemMeta meta = item.getItemMeta();// 452
            meta.setDisplayName(ChatColor.translateAlternateColorCodes('&', displayName));// 453
            if (loreLines.length != 0) {// 454
                List<String> loreCC = new ArrayList();// 455
                Arrays.stream(loreLines).forEach((l) -> {// 456
                    loreCC.add(ChatColor.translateAlternateColorCodes('&', l));
                });
                meta.setLore(loreCC);// 457
            }

            item.setItemMeta(meta);// 459
        }

        return internals.setSkullOwner(item, this.getSkullOwner());// 462
    }

    public static String getTextureValue(ItemStack item) {
        return internals.getTextureValue(item);// 517
    }

    public static ItemStack setSkullOwner(ItemStack item, UUID id, String textureValue) {
        return setSkullOwner(item, id.toString(), textureValue);// 531
    }

    public static ItemStack setSkullOwner(ItemStack item, String id, String textureValue) {
        return internals.setSkullOwner(item, internals.createOwnerCompound(id, textureValue));// 545
    }

    static {
        String packageName = HeadLib.class.getPackage().getName();// 480
        try {
            String internalsName = Bukkit.getServer().getClass().getPackage().getName().split("\\.")[3];// 481
            internals = (HeadLib.InternalsProvider)Class.forName(packageName + "." + internalsName).getDeclaredConstructor().newInstance();// 483
        } catch (InstantiationException | IllegalAccessException | ClassCastException | NoSuchMethodException | SecurityException | IllegalArgumentException | InvocationTargetException | ClassNotFoundException var3) {// 484
            internals = new Generic();
        }
        catch (ArrayIndexOutOfBoundsException ex) {
            internals = new Modern();
        }

    }// 508

    interface InternalsProvider {
        ItemStack newPlayerHead(int var1);

        String getTextureValue(ItemStack var1);

        ItemStack setSkullOwner(ItemStack var1, Object var2);

        Object createOwnerCompound(String var1, String var2);
    }
}
