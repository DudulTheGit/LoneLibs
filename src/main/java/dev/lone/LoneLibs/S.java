...
public class S
{
    public static <K> void increase(HashMap<K, Integer> map, K key)
    {
        map.merge(key, 1, Integer::sum);
    }

    public static <A, X, Y> Y mapData(HashMap<A, HashMap<X, Y>> map, A mainKey, X secondKey, Function<X, Y> addLogic)
    {
        return map.computeIfAbsent(mainKey, k -> new HashMap<>()).computeIfAbsent(secondKey, addLogic);
    }
